...
class ShinobuPhaserEffect : public ShinobuEffect {
	GDCLASS(ShinobuPhaserEffect, ShinobuEffect);
	ma_phaser_node phaser_node;

protected:
	static void _bind_methods();

public:
	ShinobuPhaserEffect(uint32_t m_channel_count);
	~ShinobuPhaserEffect();
