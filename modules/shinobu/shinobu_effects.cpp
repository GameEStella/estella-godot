...
void ShinobuPitchShiftEffect::set_pitch_scale(float m_pitch_scale) {
	ma_pitch_shift_node_set_pitch_scale(&pitch_shift_node, m_pitch_scale);
}

float ShinobuPitchShiftEffect::get_pitch_scale() {
	return ma_pitch_shift_node_get_pitch_scale(&pitch_shift_node);
}

ma_node *ShinobuPitchShiftEffect::get_node() {
	return &pitch_shift_node;
}
...
ShinobuPitchShiftEffect::ShinobuPitchShiftEffect(uint32_t channel_count) {
	...
	ma_result result = ma_pitch_shift_node_init(ma_engine_get_node_graph(engine), &pitch_shift_config, NULL, &pitch_shift_node);
	MA_ERR(result, "Error creating pitch shift effect");
}

ShinobuPitchShiftEffect::~ShinobuPitchShiftEffect() {
	ma_pitch_shift_node_uninit(&pitch_shift_node, NULL);
}

void ShinobuPhaserEffect::_bind_methods() {
    ClassDB::bind_method(D_METHOD("set_range_min_hz", "hz"), &ShinobuPhaserEffect::set_range_min_hz);
    ClassDB::bind_method(D_METHOD("get_range_min_hz"), &ShinobuPhaserEffect::get_range_min_hz);
    ClassDB::bind_method(D_METHOD("set_range_max_hz", "hz"), &ShinobuPhaserEffect::set_range_max_hz);
    ClassDB::bind_method(D_METHOD("get_range_max_hz"), &ShinobuPhaserEffect::get_range_max_hz);
    ClassDB::bind_method(D_METHOD("set_rate_hz", "hz"), &ShinobuPhaserEffect::set_rate_hz);
    ClassDB::bind_method(D_METHOD("get_rate_hz"), &ShinobuPhaserEffect::get_rate_hz);
    ClassDB::bind_method(D_METHOD("set_feedback", "fbk"), &ShinobuPhaserEffect::set_feedback);
    ClassDB::bind_method(D_METHOD("get_feedback"), &ShinobuPhaserEffect::get_feedback);
    ClassDB::bind_method(D_METHOD("set_depth", "depth"), &ShinobuPhaserEffect::set_depth);
    ClassDB::bind_method(D_METHOD("get_depth"), &ShinobuPhaserEffect::get_depth);
...
void ShinobuPhaserEffect::set_range_min_hz(float hz) {
    ma_phaser_node_set_range_min_hz(&phaser_node, hz);
}

float ShinobuPhaserEffect::get_range_min_hz() {
    return ma_phaser_node_get_range_min_hz(&phaser_node);
}

void ShinobuPhaserEffect::set_range_max_hz(float hz) {
    ma_phaser_node_set_range_max_hz(&phaser_node, hz);
}

float ShinobuPhaserEffect::get_range_max_hz() {
    return ma_phaser_node_get_range_max_hz(&phaser_node);
}

void ShinobuPhaserEffect::set_rate_hz(float hz) {
    ma_phaser_node_set_rate_hz(&phaser_node, hz);
}

float ShinobuPhaserEffect::get_rate_hz() {
    return ma_phaser_node_get_rate_hz(&phaser_node);
}

void ShinobuPhaserEffect::set_feedback(float fbk) {
    ma_phaser_node_set_feedback(&phaser_node, fbk);
}

float ShinobuPhaserEffect::get_feedback() {
    return ma_phaser_node_get_feedback(&phaser_node);
}

void ShinobuPhaserEffect::set_depth(float depth) {
    ma_phaser_node_set_depth(&phaser_node, depth);
}

float ShinobuPhaserEffect::get_depth() {
    return ma_phaser_node_get_depth(&phaser_node);
}

ma_node *ShinobuPhaserEffect::get_node() {
    return &phaser_node.baseNode;
}

ShinobuPhaserEffect::ShinobuPhaserEffect(uint32_t m_channel_count) {
    ma_engine *engine = Shinobu::get_singleton()->get_engine();
    ma_phaser_node_config phaser_config = ma_phaser_node_config_init(ma_engine_get_sample_rate(engine));
    ma_result result = ma_phaser_node_init(ma_engine_get_node_graph(engine), &phaser_config, NULL, &phaser_node);
    MA_ERR(result, "Error creating phaser effect");
}

ShinobuPhaserEffect::~ShinobuPhaserEffect() {
    ma_phaser_node_uninit(&phaser_node, NULL);
}
...
ShinobuSpectrumAnalyzerEffect::ShinobuSpectrumAnalyzerEffect(uint32_t m_channel_count) {
	...
	ma_result result = ma_spectrum_analyzer_node_init(ma_engine_get_node_graph(engine), &spectrum_config, NULL, &analyzer_node);
...
Vector2 ShinobuSpectrumAnalyzerEffect::get_magnitude_for_frequency_range(float m_begin, float m_end, ma_spectrum_magnitude_mode mode) {
	MagnitudeResult res = ma_spectrum_analyzer_get_magnitude_for_frequency_range(m_begin, m_end, mode, &analyzer_node);
